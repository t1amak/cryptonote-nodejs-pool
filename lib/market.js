...
function getExchangeMarkets (exchange, callback) {
	...
	if (exchange === "coinex") {
		...
		const poolSymbol = String(cfgSymRaw).toUpperCase();

		// exchange base: only map SAL1->SAL when Salvium is enabled
		let exchangeBase = poolSymbol;
		try {
			if (typeof utils?.isSalviumEnabled === 'function' && utils.isSalviumEnabled() && poolSymbol === 'SAL1') {
			exchangeBase = 'SAL';
			}
		} catch (_) {}

		const exCfg = (config && config.exchanges && config.exchanges.coinex) || {};
		const quotesRaw = exCfg.quotes ?? exCfg.quote ?? 'USDT';
		const quotes = Array.isArray(quotesRaw) ? quotesRaw : [quotesRaw];
		if (quotes.length === 0) quotes.push('USDT');
	...
	// MEXC (single-market request; unified pattern)
		...
		const exCfg = (config && config.exchanges && config.exchanges.mexc) || {};
		const quotesRaw = exCfg.quotes ?? exCfg.quote ?? 'USDT';
		const quotes = Array.isArray(quotesRaw) ? quotesRaw : [quotesRaw];
		if (quotes.length === 0) quotes.push('USDT');
	...
	// NonKYC.io
		...
		// Base symbol comes strictly from config (no SAL fallback).
		const cfgSymRaw = (config && config.symbol);
		if (!cfgSymRaw) {
		...
		// Exchange symbol: special-case only for Salvium -> map SAL1 (pool) to SAL (exchange).
		let exchangeBase = poolSymbol;
		try {
			if (typeof utils?.isSalviumEnabled === 'function' && utils.isSalviumEnabled() && poolSymbol === 'SAL1') {
			exchangeBase = 'SAL';
		...
		// Quotes from settings; default to USDT if none provided.
		const exCfg = (config && config.exchanges && config.exchanges.nonkyc) || {};
		const quotesRaw = exCfg.quotes ?? exCfg.quote ?? 'USDT';
		const quotes = Array.isArray(quotesRaw) ? quotesRaw : [quotesRaw];
	...
	// TradeOgre - śpij słodko aniołku U_U'
